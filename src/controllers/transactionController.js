...
const ProductModel = require("../models/productModel"); // Perlu untukmendapatkan harga produk
const CustomerModel = require("../models/customerModel"); // Perlu untukvalidasi customer
const TransactionController = {
  createTransaction: async (req, res) => {
    ...
    if (!customerId || !items || !Array.isArray(items) || items.length === 0) {
      return res
        .status(400)
        .json({ message: "Customer ID andtransaction items are required" });
    }
    try {
      // Validasi customer
      ...
      for (const item of processedItems) {
        ...
        // Kurangi stok produk (ini operasi penting dalamtransaksi!)
      ...
      res
        .status(201)
        .json({ message: "Transaction createdsuccessfully", transactionId });
    } catch (error) {
  ...
  getTransactionById: async (req, res) => {
    ...
    try {
      ...
      if (transactionItems.length === 0) {
        return res.status(404).json({ message: "Transaction notfound" });
      }
  ...
  getTransactionsByCustomerId: async (req, res) => {
    ...
    try {
      ...
      if (transactionItems.length === 0) {
        return res
          .status(404)
          .json({ message: "No transactionsfound for this customer" });
  ...
  updateTransactionStatus: async (req, res) => {
    ...
    try {
      ...
      if (affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Transaction notfound or no changes made" });
      }
      res
        .status(200)
