...
const CustomerController = {
  createCustomer: async (req, res) => {
    ...
    if (!userId || !name || !email) {
      return res
        .status(400)
        .json({ message: "User ID, name, andemail are required" });
    }
    try {
      ...
      res
        .status(201)
        .json({ message: "Customer createdsuccessfully", customerId });
    } catch (error) {
  ...
  getCustomerByUserId: async (req, res) => {
    ...
    try {
      ...
      if (!customer) {
        return res
          .status(404)
          .json({ message: "Customer not foundfor this user ID" });
      }
      res.status(200).json(customer);
    } catch (error) {
  ...
  updateCustomer: async (req, res) => {
    ...
    try {
      ...
      if (affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Customer not foundor no changes made" });
      }
