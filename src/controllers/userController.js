...
// Untuk password hashing, Anda bisa menggunakan bcrypt (npm installbcrypt)
...
const UserController = {
  registerUser: async (req, res) => {
    ...
    if (!username || !password) {
      return res
        .status(400)
        .json({ message: "Username and passwordare required" });
    }
    try {
  ...
  updateUser: async (req, res) => {
    ...
    try {
      ...
      if (affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "User not found orno changes made" });
      }
